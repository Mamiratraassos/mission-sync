export default async function handler(req, res) {
  ...
  try {
    const { text, title, location, filled, sectionKeys } = req.body || {};
    if (!text) return res.status(400).json({ error: "Missing text" });

    const validSections = sectionKeys || "contexte|objectifs|activites|observations|resultats|recommandations|prochaines_etapes|divers";
...
Sections possibles: ${validSections}
Note brute: "${text}"

Reponds UNIQUEMENT avec un objet JSON valide, sans backticks:
{"corrected":"la note corrigee en francais professionnel","section":"une des sections possibles listees ci-dessus","report_addition":"1 a 2 phrases style rapport professionnel"}`
        }],
      }),
    });
